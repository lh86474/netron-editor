...
- You add node by using the right-click, and you hover over any node you can click insert above or insert below. Netron will display a list of operators supported by the onnx intermediate representation 
	- If we're going to add a node that's questionable, such as add, which requires two inputs. Let's add it under this node here which only has one output, we will get a warning, but we can add it anyway and fix the structure of the graph later
- To add your properties, you can start adding it manually. 
- Removing the nodes is kind of similar, let's say that I want to delete an activation node, it won't really change much since it has one input and one output, so the rewiring won't be ambiguous, but if we try to delete on the conv node, which has many inputs like the weight and bias, we will get a warning first. 
- And if you want to undo or redo any changes, there's buttons on the toolbar and on the sidebar, and some shortcuts
- Now, after we have edited everything, let's extract a subgraph of the graph with just the nodes that we've edited
...
That's basically all of the editing features that we have. 
...
- The user can't choose which graph will be the upstream and which graph is the downstream
	- What upstream means is that the upstream graph, which is graph in which the output of the graph is being used, and vice versa for the downstream. To make that more clear, I can also show the source graphs so people can see where exactly the graph is being merged. I'll definitely have to work on making this more user friendly since this doesn't look too good, and it's hard for people to actually see the differences
